...
المهام المجدولة (`.github/workflows/`):
...
| `skills-verify.yml` | يوميًا 04:00 UTC | يتحقق من المهارات المنسّقة ضد مستودعاتها المصدر، يحدّث `lastVerified`، ويرقّع أي تغيير |
| `skills-discover.yml` | أسبوعيًا الأربعاء 05:00 UTC | يكتشف مهارات جديدة من `content/sources.yaml` ويولّد صفحات تجريبية (`maturity: experimental`) |
| `use-cases.yml` | أسبوعيًا الاثنين 05:00 UTC | يُحدّث حالات الاستخدام المهنية من مصادر مختارة |
| `link-rot.yml` | أسبوعيًا الأحد 06:00 UTC | يفحص الروابط الخارجية ويستبدل الروابط الميتة بالبدائل الرسمية |
| `freshness.yml` | شهريًا (أول يوم) 07:00 UTC | يعيد جلب التوثيق الرسمي للوحدات والأدلة وحالات الاستخدام المتجاوزة 90 يومًا، ويصحّح الانحراف |
| `ops-report.yml` | شهريًا (أول يوم) 09:00 UTC | تقرير عمليات يتضمّن عدد النشرات الذاتية والـ Issues وPRs والمهارات المكتشفة |
| `post-deploy-smoke.yml` | بعد كل `deploy.yml` ناجح | يفحص الموقع الحي: صفحات مفتاحية، sitemap، JSON-LD، Pagefind |
| `deploy.yml` | عند كل push إلى `main` | يبني الموقع وينشره فعليًا على السيرفر عبر rsync/SSH |

كل المهام التي تُنشئ أو تُعدّل محتوى تستخدم `scripts/_auto-publish.ts`: لا شيء يصل
إلى `main` إلا بعد اجتياز `npm run check && npm run audit && npm run build`. عند
الفشل يُفتح PR تلقائيًا للمراجعة البشرية بدل نشر شيء غير متحقَّق منه. يمكن
وضع سقف للتكلفة عبر متغيري البيئة `MAX_INPUT_TOKENS` و `MAX_OUTPUT_TOKENS`.
